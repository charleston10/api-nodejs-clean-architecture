...
const { UserEntity: userEntity } = require('src/data/local');

describe('Data :: User :: UserRepository', () => {
  let repository;

  beforeEach(() => {
    repository = new UserRepository({ userEntity });
  });

  describe('#getAll', () => {
    beforeEach(() => {
      return factory.createMany('client', 2, [
        { name: 'User 1' },
        { name: 'User 2' }
      ]);
    });
    ...
    it('returns all users from the database', async () => {
      ...
      expect(users[0]).to.be.instanceOf(UserModel);
      expect(users[0].name).to.equal('User 1');

      expect(users[1]).to.be.instanceOf(UserModel);
  ...
  describe('#getById', () => {
    context('when user exists', () => {
      it('returns the user', async () => {
        ...
        expect(foundUser).to.be.instanceOf(UserModel);
